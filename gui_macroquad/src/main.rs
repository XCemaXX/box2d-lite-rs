...
use physics::primitives::{Line, Rectangle, Point};
...
fn update_frame_physics(rectangles: Vec<Rectangle>, points: Vec<Point>, lines: Vec<Line>, size_params: &SizeParams) {
    for r in &rectangles {
        let (x, y, w, h) = rectangle_transform(r, &size_params);
        let p = DrawRectangleParams {color: DARK_GREEN, offset: Vec2{x: 0.5, y: 0.5}, rotation: -r.rotation};
        draw_rectangle_ex(x, y, w, h, p); //outer
        let p = DrawRectangleParams {color: GRAY_BACKGROUND, offset: Vec2{x: 0.5, y: 0.5}, rotation: -r.rotation};
        let frame_width = 4.0;
        draw_rectangle_ex(x, y, w - frame_width, h - frame_width, p); //inner
    }
    for c in &points {
        let (x, y) = transform_coords(c.x, c.y, &size_params);
        let p = DrawRectangleParams {color: RED, offset: Vec2{x: 0.5, y: 0.5}, rotation: 0.0};
...
fn update_physics_by_input(physics_state: &mut PhysicsState, buttons: Vec<Button>, size_params: &SizeParams) {
    ...
    if mouse_x > size_params.offset_x && mouse_x < size_params.offset_x + size_params.width &&
       mouse_y > size_params.offset_y && mouse_y < size_params.offset_y + size_params.width &&
        ...
        let pressed = 'pressed: {
            for b in &buttons {
                if b.is_cursor_in(mouse_x, mouse_y) {
                    match b.name.as_str() {
                        "Next" => { physics_state.change_to_next_scene(); },
                        "Restart" => { physics_state.restart(); },
                        "Prev" => { physics_state.change_to_prev_scene(); },
                        _ => { }
                    };
                    break 'pressed true;
                }
